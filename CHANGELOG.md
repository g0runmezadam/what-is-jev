...
- **Later the same day** — two more batches as new repositories kept appearing: 24 and then 14 (the second includes the last links shared in the community channel), 38 repositories in all (947 counted in total). Their 16 class A rows were re-read by a single reviewer: 13 stayed in class A, 3 moved to B. Two shared links had been deleted and were skipped.
- **Findings**: an independent head-to-head benchmark against reasoning-enabled models ([repository](https://github.com/manjunathshiva/jev-frontier-bench)) was checked number by number against its README and added twice — under speed, cost and accuracy (VERIFIED: only the two flagship models clearly beat Jev on 200 decisions; differences under about ten points are noise at this sample size) and under calibration (CONTESTED: worse calibration error than the strongest model, and further from 100-annotator human label splits than a blind guess). One figure from the author's announcement could not be found in the README and was left out.
- **Sources**: 90 in total. Three trust values entered earlier today used the scale upside down (5 is the most trusted, not 1) and were corrected.
- **Pipeline**: two same-day scoring files that touch the same repository now stop the run instead of letting the older one win; an audit must name the scoring batch it read, so a stale audit cannot confirm a newer score; `true` is no longer accepted where a number is expected; network-share paths, home directories and file URLs were added to the privacy patterns.
